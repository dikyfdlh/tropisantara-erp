    ...
    <form action={action}>
      <Card>
        ...
        <CardBody className="grid grid-cols-1 gap-4 md:grid-cols-2">
          ...
          <div>
            ...
            <Select name="businessType" required defaultValue={initial?.businessType ?? 'PT'}>
              {BUSINESS_TYPES.map((t) => <option key={t} value={t}>{t}</option>)}
            </Select>
          </div>
          <div><Label>NPWP</Label><Input name="npwp" defaultValue={initial?.npwp ?? ''} /></div>
        </CardBody>
      </Card>
      ...
      <Card className="mt-4">
        ...
        <CardBody className="grid grid-cols-1 gap-4 md:grid-cols-2">
          ...
          <div><Label>WhatsApp</Label><Input name="whatsapp" defaultValue={initial?.whatsapp ?? ''} placeholder="+62 ..." /></div>
      ...
      <Card className="mt-4">
        ...
        <CardBody className="grid grid-cols-1 gap-4 md:grid-cols-2">
          ...
          <div><Label>Kota / Kabupaten</Label><Input name="city" defaultValue={initial?.city ?? ''} /></div>
          <div><Label>Provinsi</Label><Input name="province" defaultValue={initial?.province ?? ''} /></div>
      ...
      <div className="mt-4 flex justify-end gap-2">
        <Link href="/master/suppliers"><Button type="button" variant="outline">Batal</Button></Link>
