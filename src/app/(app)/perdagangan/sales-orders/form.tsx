...
export type ProductOption  = { id: string; code: string; name: string; sellPrice: number; uom: string };

type Item = { productId: string; qty: number; unitPrice: number; discount: number };

export function SalesOrderForm({
  action, customers, products,
}: {
  action: (fd: FormData) => void | Promise<void>;
  customers: CustomerOption[];
  products: ProductOption[];
}) {
  const [items, setItems] = useState<Item[]>([{ productId: '', qty: 1, unitPrice: 0, discount: 0 }]);
  // PPN opsional — default 0 (tidak kena pajak). User dapat isi 11 / nilai lain bila perlu.
  const [taxRate, setTaxRate] = useState(0);

  const totals = useMemo(() => {
    const itemRows = items.map((it) => ({ ...it, total: Math.max(0, Math.round(it.qty * it.unitPrice) - it.discount) }));
    const subtotal = itemRows.reduce((s, r) => s + r.total, 0);
    const tax = Math.round((subtotal * taxRate) / 100);
    return { subtotal, tax, total: subtotal + tax };
  }, [items, taxRate]);

  function addRow() { setItems([...items, { productId: '', qty: 1, unitPrice: 0, discount: 0 }]); }
  ...
  function update(i: number, k: keyof Item, v: string | number) {
    setItems(items.map((it, idx) => {
      ...
      if (k === 'productId') {
        ...
        if (prod && !it.unitPrice) next.unitPrice = prod.sellPrice;
      }
      return next;
    }));
  }
    ...
    <form action={action}>
      <Card>
        <CardBody className="grid grid-cols-1 gap-4 md:grid-cols-3">
          ...
          <div><Label>Tanggal Kirim</Label><Input name="deliveryDate" type="date" /></div>
          <div>
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              ...
              <tbody className="divide-y divide-slate-100">
                {items.map((it, i) => {
                  const subtotal = Math.max(0, Math.round(it.qty * it.unitPrice) - it.discount);
                  return (
                    <tr key={i}>
                      ...
                      <td className="px-3 py-2 w-28">
                        <Input name="itemDiscount" type="number" min={0} value={it.discount}
                          onChange={(e) => update(i, 'discount', parseInt(e.target.value || '0', 10))} />
                      </td>
                      <td className="px-3 py-2 text-right font-medium">{formatRupiah(subtotal)}</td>
              ...
              <tfoot className="bg-slate-50 text-sm">
                <tr><td colSpan={4} className="px-3 py-2 text-right">Subtotal</td>
                  <td className="px-3 py-2 text-right font-medium">{formatRupiah(totals.subtotal)}</td><td></td></tr>
                <tr><td colSpan={4} className="px-3 py-2 text-right">PPN ({taxRate}%)</td>
                  <td className="px-3 py-2 text-right font-medium">{formatRupiah(totals.tax)}</td><td></td></tr>
                <tr><td colSpan={4} className="px-3 py-2 text-right text-base font-semibold">Total</td>
      ...
      <div className="mt-4 flex justify-end gap-2">
        <Link href="/perdagangan/sales-orders"><Button type="button" variant="outline">Batal</Button></Link>
